...
<div class="page-header">
	<h1>
		<spring:message code="label.manageAcademicTax.readAcademicTax" />
		<small></small>
	</h1>
</div>
		...
		<div class="modal-content">
			<form id="deleteForm"
				...
				<div class="modal-body">
					<p>
						<spring:message
							code="label.manageAcademicTax.readAcademicTax.confirmDelete" />
					</p>
				</div>
			...
			<table class="table">
				<tbody>
					<tr>
						...
						<td><c:out value='${academicTax.product.name.content}' /></td>
					</tr>
					<tr>
						<th scope="row" class="col-xs-3"><spring:message
								code="label.AcademicTax.appliedOnRegistration" /></th>
						<td><c:if test="${academicTax.appliedOnRegistration}">
								<spring:message code="label.true" />
							</c:if> <c:if test="${not academicTax.appliedOnRegistration}">
								<spring:message code="label.false" />
							</c:if></td>
					</tr>
					<tr>
						<th scope="row" class="col-xs-3"><spring:message
								code="label.AcademicTax.appliedOnRegistrationFirstYear" /></th>
						<td><c:if
								test="${academicTax.appliedOnRegistrationFirstYear}">
								<spring:message code="label.true" />
							</c:if> <c:if test="${not academicTax.appliedOnRegistrationFirstYear}">
								<spring:message code="label.false" />
							</c:if></td>
					</tr>
					<tr>
						<th scope="row" class="col-xs-3"><spring:message
								code="label.AcademicTax.appliedOnRegistrationSubsequentYears" /></th>
						<td><c:if
								test="${academicTax.appliedOnRegistrationSubsequentYears}">
								<spring:message code="label.true" />
							</c:if> <c:if
								test="${not academicTax.appliedOnRegistrationSubsequentYears}">
								<spring:message code="label.false" />
							</c:if></td>
					</tr>
				</tbody>

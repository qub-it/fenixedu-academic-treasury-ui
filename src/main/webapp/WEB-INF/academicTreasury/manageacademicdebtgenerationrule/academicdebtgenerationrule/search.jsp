...
<script type="text/javascript">
	  function processDelete(externalId) {
	    url = "${pageContext.request.contextPath}/academictreasury/manageacademicdebtgenerationrule/academicdebtgenerationrule/search/delete/" + externalId;
			...
			<tbody>
				<c:forEach var="rule" items="${searchacademicdebtgenerationruleResultsDataSet}">
					<tr>
							...
							<c:forEach var="entry" items="${rule.academicDebtGenerationRuleEntriesSet}">
								<p>
									...
									<c:if test="${entry.createDebt}">
										(<em><spring:message code="label.AcademicDebtGenerationRuleEntry.createDebt" /></em>)
									</c:if>
								</p>
							</c:forEach>
						</td>
						<td>
							<p>
								<span><strong><spring:message code="label.AcademicDebtGenerationRule.executionYear" />:</strong></span>
								<span>${rule.executionYear.qualifiedName}</span>
							</p>
							
							<p>
								<span><strong><spring:message code="label.AcademicDebtGenerationRule.aggregateOnDebitNote" />:</strong></span>
								<span>
						...
						<c:if test="${rule.aggregateOnDebitNote}">
							<p>
								<span><strong><spring:message code="label.AcademicDebtGenerationRule.aggregateAllOrNothing" />:</strong></span>
								<span>
						...
						<c:if test="${rule.aggregateOnDebitNote}">
							<p>
								<span><strong><spring:message code="label.AcademicDebtGenerationRule.closeDebitNote" />:</strong></span>
								<span>
						...
						<c:if test="${rule.closeDebitNote}">
							<p>
								<span><strong><spring:message code="label.AcademicDebtGenerationRule.createPaymentReferenceCode" />:</strong></span>
								<span>
						...
						<c:if test="${rule.active}">
							<p style="color:green;"><strong><spring:message code="label.AcademicDebtGenerationRule.active.message" /></strong></p>
						</c:if>

						<c:if test="${not rule.active}">
							<p style="color:red;"><strong><spring:message code="label.AcademicDebtGenerationRule.inactive.message" /></strong></p>
						</c:if>

						</td>
						<td>
							<a  class="btn btn-xs btn-danger" href="#" onClick="javascript:processDelete('${rule.externalId}')">
								<span class="glyphicon glyphicon-trash" aria-hidden="true"></span>&nbsp;<spring:message code='label.delete'/>
							</a>

						<c:if test="${rule.active}">

							<a  class="btn btn-default btn-xs" href="${pageContext.request.contextPath}<%= AcademicDebtGenerationRuleController.PROCESS_ACTION_URL %>${rule.externalId}">
								<spring:message code='label.manageacademicdebtgenerationrule.process'/>
							</a>

							<a  class="btn btn-default btn-xs" href="${pageContext.request.contextPath}<%= AcademicDebtGenerationRuleController.SEARCH_TO_INACTIVATE_ACTION_URL %>${rule.externalId}">
								<spring:message code='label.manageacademicdebtgenerationrule.inactivate'/>
							</a>
						</c:if>

						<c:if test="${not rule.active}">
							<a  class="btn btn-default btn-xs" href="${pageContext.request.contextPath}<%= AcademicDebtGenerationRuleController.SEARCH_TO_ACTIVATE_ACTION_URL %>${rule.externalId}">
...
<script>
	...
	$(document).ready(function() {
		...
		var table = $('#searchacademicdebtgenerationruleTable').DataTable({language : {
